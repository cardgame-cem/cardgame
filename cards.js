(function() {
  ...
  var config = {
    ...
    parent: 'card-game-cem' // id of the container
  };
  ...
  var y = getY();
  var x = getX();
  var [imageSizeX, imageSizeY] = getImageSizeDimensions();
  ...
  function preload() {
    const found = new Set();
    cards1960.forEach((card) => {
      this.load.image(`card_front_1960_${card[0]}`, `assets/cards/${card[0]}`);
      if (!found.has(card[1])) {
        this.load.image(`card_back_1960_${card[1]}`, `assets/cards/${card[1]}`);
        found.add(card[1]);
      }
    });
    cards2020.forEach((card) => {
      this.load.image(`card_front_2020_${card[0]}`, `assets/cards/${card[0]}`);
      if (!found.has(card[1])) {
        this.load.image(`card_back_2020_${card[1]}`, `assets/cards/${card[1]}`);
        found.add(card[1]);
      }
    });
  }
  ...
  function create() {
    const backButton = this.add.text(10, 15, 'Back to main page', { fill: '#eee' });
    backButton.setInteractive({ useHandCursor: true });
    backButton.on('pointerdown', () => window.history.back() );

    left = shuffleArray(cards1960);
    right = shuffleArray(cards2020);

    fontSize = height/10;

    if (!isMobile()) {
      buttonLeft = this.add
        .text(fontSize, height - 1.5*fontSize, 'i', { fill: '#ef7f66', fontSize: fontSize, align: 'center' })
        .setOrigin(0.5, 0);
      buttonLeft.setInteractive();
      buttonLeft.on('pointerdown', () => showInfoLeft() );

      buttonRight = this.add
        .text(width - fontSize, height - 1.5*fontSize, 'i', { fill: '#a2d1ac', fontSize: fontSize, align: 'center' })
        .setOrigin(0.5, 0);
  ...
  function nextImageLeft() {
    ...
    imageLeft = this.add
      .image(x, y, `card_back_1960_${left[0][1]}`)
      .setInteractive();
    imageLeft.setDisplaySize(imageSizeX, imageSizeY);
    imageLeft.on('pointerdown', function () {
  ...
  function nextImageRight() {
    ...
    imageRight = this.add
      .image(width - x, y, `card_back_2020_${right[0][1]}`)
      .setInteractive();
    imageRight.setDisplaySize(imageSizeX, imageSizeY);
    imageRight.on('pointerdown', function () {
  ...
  function update ()
  {
    ...
    if (animationLeftStarted !== 'none') {
      if (animationLeftStarted === 'decrease') {
        imageLeft.scaleX -= tick;
        if (imageLeft.scaleX <= 0) {
          animationLeftStarted = 'increase'
          imageLeft.destroy();
          imageLeft = this.add
            .image(x, y, `card_front_1960_${left[0][0]}`)
            .setInteractive();
          left.shift();
    ...
    if (animationRightStarted !== 'none') {
      if (animationRightStarted === 'decrease') {
        imageRight.scaleX -= tick;
        if (imageRight.scaleX <= 0) {
          animationRightStarted = 'increase'
          imageRight.destroy();
          imageRight = this.add
            .image(width - x, y, `card_front_2020_${right[0][0]}`)
            .setInteractive();
          right.shift();
  ...
  window.addEventListener('resize', () => {
    ...
    x = getX();
    y = getY();
    [imageSizeX, imageSizeY] = getImageSizeDimensions();
    imageLeft.setDisplaySize(imageSizeX, imageSizeY);
    imageRight.setDisplaySize(imageSizeX, imageSizeY);
    imageLeft.setPosition(x, y);
    imageRight.setPosition(width - x, y);
    
    fontSize = height/10;
    if (buttonLeft && buttonRight) {
      buttonLeft.setFontSize(fontSize);
      buttonRight.setFontSize(fontSize);
      buttonLeft.setPosition(fontSize, height - 1.5*fontSize);
      buttonRight.setPosition(width - fontSize, height - 1.5*fontSize);
    }
  });
  ...
  function getImageSizeDimensions() {
    if (height * 1.5 > width) {
      const imageSizeX = width / 2 / 1.5;
      return [imageSizeX, imageSizeX / getImageRatio()];  
    }
    const imageSizeY = height / 1.5;
    return [imageSizeY * getImageRatio(), imageSizeY];
  }
